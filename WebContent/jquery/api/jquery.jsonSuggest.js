...
function selectResultItem(item){
	if(typeof settings.functionBean != null && typeof settings.functionBean == 'function'){
		obj.val(settings.functionBean(item)); 
...
for(i=0;i<resultObjects.length;i+=1){
	var item=$('<div />');
	if(typeof settings.functionBean != null && typeof settings.functionBean == 'function'){
		text= settings.functionBean(resultObjects[i]); 
	}else if(typeof resultObjects[i] == 'string'){
		text=resultObjects[i].toString();
	}else{
...
$(item).addClass('resultItem').addClass((bOddRow)?'odd':'even').click(function(n){
	return function(){	selectResultItem(resultObjects[n]);	}
	;}(i)).mouseover(function(el){return function(){setHoverClass(el);};}(item));$(results).append(item);bOddRow=!bOddRow;iFound+=1;if(typeof settings.maxResults==='number'&&iFound>=settings.maxResults){break;}}
